...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const [left, right] = a > b ? [b, a] : [a, b];
  const result = `${isStartIncluded ? '[' : '('}${left}, ${right}${
    isEndIncluded ? ']' : ')'
  }`;
  return result;
}
...
function isCreditCardNumber(ccn) {
  let resultStr = ccn.toString();
  const { length } = resultStr;
  let i = 0;
  let item = 0;
  if (length % 2 !== 0) i = 1;
  for (; i < length; i += 2) {
    ...
    resultStr = resultStr
      .slice(0, i)
      .concat(item)
      .concat(resultStr.slice(i + 1));
  }
  return resultStr.split('').reduce((pv, cv) => +pv + +cv) % 10 === 0;
}
...
function getCommonDirectoryPath(paths) {
  const doPath = paths.map((path) => path.split('/'));
  const [firstP] = doPath;
  let result = '';
  firstP.forEach((dir, dirIdx) => {
    const yes = doPath.every((path) => path[dirIdx] === dir);
  ...
  for (let i = 0; i < m1.length; i += 1) {
    ...
    for (let j = 0; j < m2[0].length; j += 1) {
      ...
      for (let k = 0; k < m2.length; k += 1) {
        const x = m1[i][k] * m2[k][j];
        sum.push(x);
      }
      const v = sum.reduce((prev, next) => prev + next, 0);
      row.push(v);
    }
    result.push(row);
  }
