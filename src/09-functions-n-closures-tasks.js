...
function getPolynom(...args) {
  ...
  return function polynom(x) {
    const ex = [...Array(args.length).keys()].reverse();
    const result = args.map((n, i) => n * x ** ex[i]);
...
function logger(func, logFunc) {
  return (...args) => {
    const start = `${func.name}(${args.map(JSON.stringify).join(',')}) starts`;
    logFunc(start);
    const result = func(...args);
    const end = `${func.name}(${args.map(JSON.stringify).join(',')}) ends`;
    logFunc(end);
    return result;
  };
}
